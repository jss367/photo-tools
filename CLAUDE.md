...
- `vireo/web/pipeline.py` — Pipeline launch: `/api/jobs/pipeline`, `/api/pipeline/{plan,slots,config,save-grouping-defaults,extract-readiness}`, and `/api/processes` CRUD. Built by `create_pipeline_blueprint(...)`. The after-import chain (`enqueue_process_job`, `chain_after_move`) lives in `vireo/services/pipeline_launch.py` as `PipelineChain`, alongside `apply_no_model_auto_skip` and `resolve_remote_archive_target`; CPU-only runtime warnings are in `vireo/runtime_warnings.py`.
