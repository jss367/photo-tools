...
def _disable_startup_backfill_timers(monkeypatch):
    """Keep deferred startup maintenance from leaking into later tests.

    Thumbnail-path backfill and other startup tasks can otherwise run against
    stale test databases after their owning test has finished. Tests for these
    paths invoke their kickoff directly or explicitly enable startup timers.
    """
